...
// The HTTP servers run inside the `node:*` Docker image (see lib/bench.js), so
// `node-http` should be attributed to that image's Node, not the host's. The
// benchmark run writes the image version here; fall back to the host otherwise.
let nodeHttpVersion = process.version
try {
  nodeHttpVersion = readFileSync(resolve('.docker-node-version'), 'utf8').trim() || process.version
} catch {
